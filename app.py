...
st.markdown(
    ...
    .section-divider {
        border-top: 1px solid rgba(216, 222, 233, 0.8);
    ...
    div[data-testid="stFormSubmitButton"] {
        /* nudge the button container so the icon aligns with the text input */
        margin-top: 26px;
    }
    ...
    div[data-testid="stFormSubmitButton"] button {
        ...
        padding: 0;
        border-radius: 10px;
        background: #a6b9ff; /* soft periwinkle */
        border: none;
        color: transparent; /* hide the text label, we'll show SVG */
        font-size: 18px;
        font-weight: 700;
        box-shadow: none;
        display: inline-grid;
    ...
    /* Hover: only darken background, do NOT scale or enlarge icon */
    ...
    /* Active/click: slightly darker, maintain icon size */
    div[data-testid="stFormSubmitButton"] button:active {
        background-color: #5f79e6; /* darker blue */
        transform: none;
        box-shadow: none;
    }
...
if not submit_button:
    ...
    if judul.strip() == "" or ringkasan.strip() == "":
        ...
        with st.spinner("Sedang melakukan klasifikasi DDC..."):
            ...
            hasil_klasifikasi = [
                {"kode": "000", "nama": "Ilmu Komputer, Informasi, dan Karya Umum", "persen": "85%"},
                {"kode": "330", "nama": "Ekonomi", "persen": "55%"},
                {"kode": "510", "nama": "Matematika", "persen": "15%"},
            ]

        rows_html = "".join(
            [
                f'<div class="result-row"><div class="result-label">{item["kode"]} - {item["nama"]}</div><div class="result-score">{item["persen"]}</div></div>'
                for item in hasil_klasifikasi
            ]
        )
